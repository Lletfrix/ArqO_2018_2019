...
for ((N = Ninicio ; N <= Nfinal ; N += Npaso)); do
    ...
    multTime=$(valgrind --tool=cachegrind --cachegrind-out-file=$fCACHE ./mult $N 2> /dev/null | grep 'time' | awk '{print $3}')
	multMisses=$(cg_annotate $fCACHE | head -n 30 | awk '{if($NF == "TOTALS") printf("%s %s\n", $5, $8)}' | sed 's/,//g')
    rm $fCACHE
    transTime=$(valgrind --tool=cachegrind --cachegrind-out-file=$fCACHE ./multT $N 2> /dev/null | grep 'time' | awk '{print $3}')
